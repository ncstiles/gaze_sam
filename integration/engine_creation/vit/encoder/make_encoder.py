...
class EncoderModel(nn.Module):
    ...
    def __init__(
        ...
        print("sam resize dimensions:", self.model.image_size[1])
        self.resize_transform = SamResize(size=512)
    ...
    def forward(self, input_image):
        if self.use_preprocess:
            print("using built in preprocessing")
            input_image = self.preprocess(input_image)
        image_embeddings = self.image_encoder(input_image)
        return image_embeddings

    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        print("in preprocess:", x.shape)
        # Resize & Permute to (C,H,W)
        x = self.resize_transform(x)
        print("after resize transform:", x.shape)
...
def run_export(
    ...
    if use_preprocess:
        print("preprocessing in the model!")
        image_size = [720, 1280]
        dummy_input = {
            "input_image": torch.randint(
                0, 255, (image_size[0], image_size[1], 3), dtype=torch.float
            )
        }
        dynamic_axes = {
        ...
        print("not preprocessing in the model")
        image_size = [512, 512]
        dummy_input = {
            "input_image": torch.torch.randn(
